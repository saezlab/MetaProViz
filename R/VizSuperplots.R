...
VizSuperplot <- function(Input_data,
  ...
  if(is.null(Superplot)==FALSE){
    if(Superplot %in% colnames(Input_SettingsFile) == FALSE){
      stop("Superplot is active. However, the column ", paste(Superplot), " is not the in Input_SettingsFile." )
    }
  }
  ...
  if(is.null(Selected_Conditions)==FALSE){
    for (Conditions in Selected_Conditions){
      if(Conditions %in% Input_SettingsFile$Conditions==FALSE){
  ...
  if(is.null(Selected_Comparisons)==FALSE){
    for (Comp in Selected_Comparisons){
      if(Comp[1] %in% Input_SettingsFile$Conditions ==FALSE){
      ...
      if(Comp[2] %in% Input_SettingsFile$Conditions ==FALSE){
