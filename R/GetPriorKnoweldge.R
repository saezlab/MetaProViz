...
LoadKEGG <- function(){
    ...
    File_path <-paste0(directory, "/KEGG_Metabolite.rds")

    if (file.exists(File_path)) {
        ## first we will check the users chache directory and whether there 
        ## are rds files with KEGG_pathways already:
        ...
        if (length(new.packages)) ## EDIT: what about if (!require("KEGGREST", quietly = TRUE)) install.packages("KEGGREST") ## EDIT: I would advise against installing some packages from within a fct, normally this should be controlled via your NAMESPACE
            install.packages(new.packages)

        suppressMessages(library(KEGGREST)) ## EDIT: same here, this should be controlled via the NAMESPACE
        ...
        KEGG_H <- data.frame(KEGGPathway = character(nrow(Pathways_H)),
            ...
            KEGG_CompoundID = seq_len(nrow(Pathways_H)),
            stringsAsFactors = FALSE)
    
        ## 3. Iterate over each pathway and extract the needed information
        for (k in seq_len(nrow(Pathways_H))) {
        ...
        KEGG_Metabolite <- separate_longer_delim(KEGG_H_Select[, -5], 
            c(Compound, KEGG_CompoundID), delim = ";")
    
        ## 5. Remove Metabolites
        ## 5.1. Ions should be removed
        Remove_Ions <- c("Calcium cation", "Potassium cation", "Sodium cation",
            "H+", "Cl-", "Fatty acid", "Superoxide", "H2O", "CO2", "Copper", 
            "Fe2+", "Magnesium cation", "Fe3+",  "Zinc cation", "Nickel", "NH4+")
        ## 5.2. Unspecific small molecules
        Remove_Small <- c("Nitric oxide", "Hydrogen peroxide", "Superoxide", 
            "H2O", "CO2", "Hydroxyl radical", "Ammonia", "HCO3-", "Oxygen", 
            "Diphosphate", "Reactive oxygen species", "Nitrite", "Nitrate", 
            "Hydrogen", "RX", "Hg")
        ...
        saveRDS(KEGG_Metabolite, file = paste0(directory, "/KEGG_Metabolite.rds"))
    }

    ## return into environment ## EDIT: I would advise against assigning an object to the Global env, lets assume there is already a "precious" object with that same name, it will be lost, why not just return the object?
...
LoadHallmarks <- function() {
    ...
    assign("Hallmark_Pathways", Hallmark, envir = .GlobalEnv) ## EDIT: I would advise against assigning an object to the Global env, lets assume there is already a "precious" object with that same name, it will be lost, why not just return the object?
}
...
LoadGaude <- function() {
    ...
    ## Return into environment
    assign("Gaude_Pathways", MetabolicSig, envir = .GlobalEnv) ## EDIT: I would advise against assigning an object to the Global env, lets assume there is already a "precious" object with that same name, it will be lost, why not just return the object?
}
...
LoadRAMP <- function(version = "2.5.4",
    ...
    ######################################################
    ## get the directory and filepath of cache results of R
    ## get chache directory
    directory <- rappdirs::user_cache_dir()
    File_path <-paste0(directory, "/RaMP-ChemicalClass_Metabolite.rds")
    ...
    if (file.exists(File_path)) {
        ...
        Structure <- OmnipathR::ramp_table("metabolite_class", version = version) ## EDIT: is the version the package version? Is there any reason to use a specific version here?
        Class <- OmnipathR::ramp_table("chem_props", version = version)

        HMDB_ChemicalClass <- merge(Structure, Class[, c(1:3, 10)], 
                by = "ramp_id", all.x = TRUE) %>%
            ## select HMDB only!
            dplyr::filter(stringr::str_starts(class_source_id, "hmdb:")) %>% 
            ## select HMDB only!
            dplyr::filter(stringr::str_starts(chem_source_id, "hmdb:")) %>% 
            ...
            ## remove 'hmdb:' prefix
            dplyr::mutate(
                class_source_id = stringr::str_remove(class_source_id, "^hmdb:")) %>% 
            ## reorder columns
            dplyr::select(class_source_id, common_name, 
                ClassyFire_class, ClassyFire_super_class, ClassyFire_sub_class) 

        #Save the results as an RDS file in the Cache directory of R
        if (!dir.exists(directory)) {
            dir.create(directory)
        }
        saveRDS(HMDB_ChemicalClass, 
            file = paste0(directory, "/RaMP-ChemicalClass_Metabolite.rds"))
    }
  ...
  ## return into environment
  assign("ChemicalClass_MetabSet", HMDB_ChemicalClass, envir = .GlobalEnv) ## EDIT: I would advise against assigning an object to the Global env, lets assume there is already a "precious" object with that same name, it will be lost, why not just return the object?
}
...
Make_GeneMetabSet <- function(Input_GeneSet,
    ...
    ## ------------ Check Input files ----------- ##
    ## 1. The input data:
    if(!is.data.frame(Input_GeneSet)) {
    ...
    if ("Target" %in% names(SettingsInfo)) {
        if (!SettingsInfo[["Target"]] %in% colnames(Input_GeneSet)) {
            stop("The ", SettingsInfo[["Target"]], 
                " column selected as Conditions in SettingsInfo was not found in Input_GeneSet. Please check your input.")
        }
    } else {
    ...
    meta_network_metabs <- meta_network[
        grepl("Metab__", meta_network$source) | grepl("Metab__HMDB", meta_network$target), -2]
    ## extract entries with genes in source or Target
    meta_network_metabs <- meta_network_metabs[
        grepl("Gene", meta_network_metabs$source) | grepl("Gene", meta_network_metabs$target),]

    ## get reactant and product
    meta_network_metabs_reactant <-  meta_network_metabs[
            grepl("Metab__HMDB", meta_network_metabs$source), ] %>% 
        dplyr::rename("metab"=1, "gene"=2)
    meta_network_metabs_products <-  meta_network_metabs[
            grepl("Metab__HMDB", meta_network_metabs$target), ] %>% 
        dplyr::rename("gene" = 1, "metab" = 2)

    meta_network_metabs <- as.data.frame(
        rbind(meta_network_metabs_reactant, meta_network_metabs_products))
    ...
    MetabSet <- merge(meta_network_metabs, Input_GeneSet, by.x = "gene", 
        by.y = SettingsInfo[["Target"]])

    ## combine with pathways --> File that can be used for combined pathway analysis (metabolites and gene t.vals)
    GeneMetabSet <- unique(as.data.frame(
        rbind(dplyr::rename(Input_GeneSet, "feature" = SettingsInfo[["Target"]]), 
            dplyr::rename(MetabSet[,-1], "feature" = 1))))
...
LoadMetalinks <- function(types = NULL,
    ...
    #------------------------------------------------------------------
    if (!any(
        c(types, cell_location, tissue_location, biospecimen_location, disease, 
            pathway, hmdb_ids, uniprot_ids) == "?")) {
        
        ## check Input parameters
        
    }

    ## Python version enables the user to add their own link to the database 
    ## dump (probably to obtain a specific version. Lets check how the link
    ## was generated and see if it would make sense for us to do the same.)
    ## --> At the moment arbitrary!
    ## we could provide the user the ability to point to their own path were 
    ## they already dumpled/stored QA version of metalinks they like to use!
    ...
    if (file.exists(File_path)) {
        ## First we will check the users chache directory and whether there are rds files with KEGG_pathways already:
        ...
        if(length(new.packages)) 
            install.packages(new.packages) ## EDIT: see above, controll via NAMESPACE
        ...
        ##WB: This mode is used for writing binary files. It opens the destination file for writing in binary mode.
        download.file(metalinks_db_url, destfile = File_path , mode = "wb")
    ...
    if(any(
        c(types, cell_location, tissue_location, biospecimen_location, disease, 
            pathway, hmdb_ids, uniprot_ids) == "?")) { ## EDIT: this vector is used several times, should be defined once and then object be used
    ...
    if (!is.null(disease)) {
        Disease <- TablesList[["disease"]]
        ## Filter the disease
        Disease <- Disease[Disease$disease %in% disease, ]
    ...
    if (!is.null(pathway)) {
        ...
        Pathway <- TablesList[["pathway"]]
        ## filter the pathway
        Pathway <- Pathway[Pathway$pathway %in% pathway, ]
    ...
    ## add other ID types:
    ## Metabolite Name
    MetalinksDB <- merge(MetalinksDB, TablesList[["metabolites"]], 
        by = "hmdb", all.x = TRUE)

    ## Gene Name
    MetalinksDB <- merge(MetalinksDB, TablesList[["proteins"]], 
