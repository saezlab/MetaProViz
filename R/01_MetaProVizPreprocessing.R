...
MetaProVizPreprocessing <- function(Input_data=data,
  ...
  k=1
  a=1
  for (m in 1:Outlier_filtering_loop){   # here we do 10 rounds of hotelling filtering
    ...
    pca_outlier <- ggplot(data = dtp) +
      ...
      ggtitle(paste("PCA outlier test filtering round ",m))+
      theme_classic()+
    ...
    screeplot<- factoextra::fviz_screeplot(PCA.res, main = paste("PCA Explained variance plot filtering round ",m, sep = ""),
                               addlabels=TRUE,
                               ncp = 20,
                               geom = c("bar", "line"),
                               barfill = "grey",
    ...
    hotelling_qcc <- qcc::mqcc(data_hot, type="T2.single",labels=rownames(data_hot),confidence.level = 0.99, title=paste("Outlier filtering via HotellingT2 test filtering round ",m,", with 99% Confidence",  sep = ""), plot= FALSE)
    ...
    HotellingT2plot <- HotellingT2plot + ggtitle(paste("Outlier filtering via Hotelling ", hotelling_qcc$type ," test filtering round ",m,", with ", 100*hotelling_qcc$confidence.level,"% Confidence"))
    ...
    if (length(hotelling_qcc[["violations"]][["beyond.limits"]]) ==0){ # loop for outliers until no outlier is detected
      ...
      sample_outliers[m] <- list(hotelling_qcc[["violations"]][["beyond.limits"]])
 ...
 # pdf(file= paste("Results_", Sys.Date(),"/","Preprocessing", "/Outlier_testing.pdf", sep = ""), onefile = TRUE ) # or multivariate quality control chart
  ...
  if(length(potent_Outlier)>0){
    total_outliers[["Potential_Outlier"]] <- list(n_potent_outlier)
  }
  ...
  if(is.null(Analytical_Replicates)!=TRUE){
    ...
    data_norm_filtered_sumed<-as.data.frame( data_norm_filtered_sumed %>%
                                               group_by(Biological_Replicates,Analytical_Replicates, Conditions) %>%
                                               summarise_all("mean"))
  }
