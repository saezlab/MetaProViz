...
feature_filtering <- function(
    ...
    # Only take feature metadata from the SE. For a data frame input a caller
    # may have passed metadata_feature directly (used by featurefilt =
    # "Manual"), and it must not be overwritten with the NULL that
    # prepare_se_input() returns for non-SE input.
    if (inherits(input_data, "SummarizedExperiment")) {
