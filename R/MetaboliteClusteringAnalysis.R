...
#' @param metadata_info_C1  \emph{Optional: } Pass ColumnNames and Cutoffs for condition 1 including the value column (e.g. Log2FC, Log2Diff, t.val, etc) and the stats column (e.g. p.adj, p.val). This must include: c(ValueCol=ColumnName_data_C1,StatCol=ColumnName_data_C1, StatCutoff= NumericValue, ValueCutoff=NumericValue) \strong{Default=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1)}
#' @param metadata_info_C2  \emph{Optional: } Pass ColumnNames and Cutoffs for condition 2 includingthe value column (e.g. Log2FC, Log2Diff, t.val, etc) and the stats column (e.g. p.adj, p.val). This must include: c(ValueCol=ColumnName_data_C2,StatCol=ColumnName_data_C2, StatCutoff= NumericValue, ValueCutoff=NumericValue)\strong{Default=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1)}
...
MCA_2Cond <- function(data_C1,
                      data_C2,
                      metadata_info_C1=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1),
                      metadata_info_C2=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1),
  ...
  Cond1_DF <-Cond1_DF%>%
    dplyr::mutate(Cutoff = dplyr::case_when(Cond1_DF$PadjCol <= as.numeric(metadata_info_C1[["StatCutoff"]]) & Cond1_DF$ValueCol > as.numeric(metadata_info_C1[["ValueCutoff"]]) ~ 'UP',
                              Cond1_DF$PadjCol <= as.numeric(metadata_info_C1[["StatCutoff"]]) & Cond1_DF$ValueCol < - as.numeric(metadata_info_C1[["ValueCutoff"]]) ~ 'DOWN',
                              TRUE ~ 'No Change'))%>%
    dplyr::mutate(Cutoff_Specific = dplyr::case_when(Cutoff == "UP" ~ 'UP',
                                       Cutoff == "DOWN" ~ 'DOWN',
                                       Cutoff == "No Change" & Cond1_DF$PadjCol <= as.numeric(metadata_info_C1[["StatCutoff"]]) & Cond1_DF$ValueCol > 0 ~ 'Significant Positive',
                                       Cutoff == "No Change" & Cond1_DF$PadjCol <= as.numeric(metadata_info_C1[["StatCutoff"]]) & Cond1_DF$ValueCol < 0 ~ 'Significant Negative',
                                       Cutoff == "No Change" & Cond1_DF$PadjCol > as.numeric(metadata_info_C1[["StatCutoff"]]) ~ 'Not Significant',
                                       TRUE ~ 'NA'))

  Cond2_DF <- Cond2_DF%>%
    dplyr::mutate(Cutoff = dplyr::case_when(Cond2_DF$PadjCol <= as.numeric(metadata_info_C2[["StatCutoff"]]) & Cond2_DF$ValueCol > as.numeric(metadata_info_C2[["ValueCutoff"]]) ~ 'UP',
                              Cond2_DF$PadjCol <= as.numeric(metadata_info_C2[["StatCutoff"]]) & Cond2_DF$ValueCol < - as.numeric(metadata_info_C2[["ValueCutoff"]]) ~ 'DOWN',
                              TRUE ~ 'No Change')) %>%
    dplyr::mutate(Cutoff_Specific = dplyr::case_when(Cutoff == "UP" ~ 'UP',
                                       Cutoff == "DOWN" ~ 'DOWN',
                                       Cutoff == "No Change" & Cond2_DF$PadjCol <= as.numeric(metadata_info_C2[["StatCutoff"]]) & Cond2_DF$ValueCol > 0 ~ 'Significant Positive',
                                       Cutoff == "No Change" & Cond2_DF$PadjCol <= as.numeric(metadata_info_C2[["StatCutoff"]]) & Cond2_DF$ValueCol < 0 ~ 'Significant Negative',
                                       Cutoff == "No Change" & Cond2_DF$PadjCol > as.numeric(metadata_info_C2[["StatCutoff"]]) ~ 'Not Significant',
                                       TRUE ~ 'NA'))
...
#' @param metadata_info_Intra  \emph{Optional: } Pass ColumnNames and Cutoffs for the intracellular metabolomics including the value column (e.g. Log2FC, Log2Diff, t.val, etc) and the stats column (e.g. p.adj, p.val). This must include: c(ValueCol=ColumnName_data_Intra,StatCol=ColumnName_data_Intra, StatCutoff= NumericValue, ValueCutoff=NumericValue) \strong{Default=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1)}
#' @param metadata_info_core  \emph{Optional: } Pass ColumnNames and Cutoffs for the consumption-release metabolomics including the direction column, the value column (e.g. Log2Diff, t.val, etc) and the stats column (e.g. p.adj, p.val). This must include: c(DirectionCol= ColumnName_data_core,ValueCol=ColumnName_data_core,StatCol=ColumnName_data_core, StatCutoff= NumericValue, ValueCutoff=NumericValue)\strong{Default=c(DirectionCol="core", ValueCol="Log2(Distance)",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1)}
#' @param FeatureID \emph{Optional: } Column name of Column including the Metabolite identifiers. This MUST BE THE SAME in each of your Input files. \strong{Default="Metabolite"}
...
MCA_core <- function(data_Intra,
                     data_core,
                     metadata_info_Intra=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1),
                     metadata_info_core=c(DirectionCol="core", ValueCol="Log2(Distance)",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=1),
                     FeatureID= "Metabolite",
                     save_table = "csv",
  ...
  #Assign to Group based on individual Cutoff ("UP", "DOWN", "No Change")
  core_DF <- core_DF%>%
    dplyr::mutate(Cutoff = dplyr::case_when(core_DF$PadjCol <= as.numeric(metadata_info_core[["StatCutoff"]]) & core_DF$ValueCol > as.numeric(metadata_info_core[["ValueCutoff"]]) ~ 'UP',
                              core_DF$PadjCol <= as.numeric(metadata_info_core[["StatCutoff"]]) & core_DF$ValueCol < - as.numeric(metadata_info_core[["ValueCutoff"]]) ~ 'DOWN',
                              TRUE ~ 'No Change')) %>%
    dplyr::mutate(Cutoff_Specific = dplyr::case_when(Cutoff == "UP" ~ 'UP',
                                       Cutoff == "DOWN" ~ 'DOWN',
                                       Cutoff == "No Change" & core_DF$PadjCol <= as.numeric(metadata_info_core[["StatCutoff"]]) & core_DF$ValueCol > 0 ~ 'Significant Positive',
                                       Cutoff == "No Change" & core_DF$PadjCol <= as.numeric(metadata_info_core[["StatCutoff"]]) & core_DF$ValueCol < 0 ~ 'Significant Negative',
                                       Cutoff == "No Change" & core_DF$PadjCol > as.numeric(metadata_info_core[["StatCutoff"]]) ~ 'Not Significant',
                                       TRUE ~ 'NA'))

  Intra_DF <-Intra_DF%>%
    dplyr::mutate(Cutoff = dplyr::case_when(Intra_DF$PadjCol <= as.numeric(metadata_info_Intra[["StatCutoff"]]) & Intra_DF$ValueCol > as.numeric(metadata_info_Intra[["ValueCutoff"]]) ~ 'UP',
                              Intra_DF$PadjCol <= as.numeric(metadata_info_Intra[["StatCutoff"]]) & Intra_DF$ValueCol < - as.numeric(metadata_info_Intra[["ValueCutoff"]]) ~ 'DOWN',
                              TRUE ~ 'No Change'))%>%
    dplyr::mutate(Cutoff_Specific = dplyr::case_when(Cutoff == "UP" ~ 'UP',
                                       Cutoff == "DOWN" ~ 'DOWN',
                                       Cutoff == "No Change" & Intra_DF$PadjCol <= as.numeric(metadata_info_Intra[["StatCutoff"]]) & Intra_DF$ValueCol > 0 ~ 'Significant Positive',
                                       Cutoff == "No Change" & Intra_DF$PadjCol <= as.numeric(metadata_info_Intra[["StatCutoff"]]) & Intra_DF$ValueCol < 0 ~ 'Significant Negative',
                                       Cutoff == "No Change" & Intra_DF$PadjCol > as.numeric(metadata_info_Intra[["StatCutoff"]]) ~ 'Not Significant',
                                       TRUE ~ 'NA'))
