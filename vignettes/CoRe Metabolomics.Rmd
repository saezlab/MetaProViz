...
```{r}
Pool_Estimation_result<- MetaProViz::Pool_Estimation(Input_data = Media[,-c(1:5)],
                            Input_SettingsFile = Media[,1:5],
...
#Prepare the input:
Media_input <- Media%>%
  subset(!Conditions=="Pool", select = -c(1:5))#remove pool samples and remove the information columns

Media_Metadata <- Media%>%
  subset(!Conditions=="Pool", select = c(1:5))#remove pool samples and keep the information columns only
...
PreProcessing_res <-  MetaProViz::Preprocessing(Input_data=Media_input,
                                                ...
                                                TIC_Normalization = TRUE,# As we have raw data we will perform total ion count norm
...
Media_Preprocessed <-Media_Preprocessed%>%
  ...
  remove_rownames()%>%column_to_rownames("Sample_ID")#Use SampleID for the rownames!
```
\
...
DMA_Annova <-  MetaProViz::DMA(Input_data=Media_Preprocessed[,-c(1:6)],
                               Input_SettingsFile_Sample=Media_Preprocessed[,c(1:6)],
                               Input_SettingsInfo = c(conditions="Conditions", numerator=NULL, denominator = "HK2"),
                               STAT_pval ="aov",
                               STAT_padj="fdr",
...
for(comparison in comparisons){
  ...
  DM_ORA_res[[comparison]] <- MetaProViz::MC_ORA(Input_data=DMA,
                   MetabCluster_lab="CoRe_specific",#column including information on the 
...
MC_ORA_786M1A_vs_HK2_Consumed <- DM_ORA_res[["786-M1A_vs_HK2"]][["Consumed"]]
```
```{r, echo=FALSE}
# Check how our data looks like:
...
MetaProViz::toy_data(data="Standard_DMA")

#Perform metabolite clustering:
MCA_CoRe_res <- MetaProViz::MCA_CoRe(Intra_File=Intra_DMA_786M1A_vs_HK2 ,
                     CoRe_File= DMA_786M1A_vs_HK2,
...
#Lets check how the results look like:
MCA_res <- MCA_CoRe_res[["DF"]][["MCA_result"]]
ClusterSummary <- MCA_CoRe_res[["DF"]][["ClusterSummary"]]
```
```{r, echo=FALSE}
# Check how our data looks like:
...
MC_ORA_result<- MetaProViz::MC_ORA(Input_data=MCA_CoRe_res[["DF"]][["MCA_result"]],
                   MetabCluster_lab="RG2_Significant",#The cluster we want to use to perform the pathway analysis on
...
#Lets check how the results look like:
Both_UP_Released <- MC_ORA_result[["Both_UP (Released)"]]
```
```{r, echo=FALSE}
# Check how our data looks like:
...
```{r}
MetaData_Metab <-MappingInfo[,c(1,4)]
```
```{r, echo=FALSE}
# Check how our data looks like:
...
```{r}
Input_PCA <- Media_Preprocessed[,-c(1:6)] #remove columns that include Metadata such as cell type,...
```
```{r, echo=FALSE}
# Check how our data looks like:
Input_PCA[1:6,1:7]%>%
...
MetaProViz::VizPCA(Plot_SettingsInfo= c(color="Biological_Replicates"),
                   ...
                   OutputPlotName = "Batch Effect (continuous color scale)")
```
...
```{r,eval=FALSE}
MetaProViz::VizPCA(Plot_SettingsInfo= c(color="Conditions", shape="Status"),
                   Plot_SettingsFile= MetaData_Sample,
                   Input_data=Input_PCA,
                   color_palette= NULL,#pass your own colour palette
                   color_scale=="discrete",#scale continuous or discrete
...
MetaProViz::VizHeatmap(Input_data = Input_Heatmap,
                       ...
                       Plot_SettingsInfo = c(color_Sample = list("Conditions","Biological_Replicates", "Status")),
...
MetaProViz::VizHeatmap(Input_data = Input_Heatmap, 
                       ...
                       Plot_SettingsInfo = c(individual_Metab = "Pathway",
                                             color_Sample = list("Conditions","Biological_Replicates"), color_Metab = list("Pathway")),
...
MetaData_Metab <- merge(MetaData_Metab, DMA_786M1A_vs_HK2[,c(1,5:6)], by="Metabolite", all.y=TRUE)
```
```{r, echo=FALSE}
# Check how our data looks like:
MetaData_Metab[c(3:4,6,24:26),]%>%
...
MetaProViz::VizVolcano(Plot_Settings="Standard",
                       Plot_SettingsInfo= c(color="CoRe_specific"),
...
MetaProViz::VizVolcano(Plot_Settings="Standard",
                       Plot_SettingsInfo= c(shape="CoRe_specific"),
                       Plot_SettingsFile= MetaData_Metab,
                       Input_data=DMA_786M1A_vs_HK2,
                       x= "Log2(Distance)",
                       OutputPlotName= "786M1A versus HK2",
...
MetaProViz::VizVolcano(Plot_Settings="Standard",
                       Plot_SettingsInfo= c(shape="CoRe_specific", color="CoRe_specific"),
                       Plot_SettingsFile= MetaData_Metab,
                       Input_data=DMA_786M1A_vs_HK2,
                       x= "Log2(Distance)",
                       OutputPlotName= "786M1A versus HK2",
...
MetaProViz::VizVolcano(Plot_Settings="Standard",
                       Plot_SettingsInfo= c(individual="CoRe", shape="CoRe_specific"),
                       Plot_SettingsFile= MetaData_Metab,
                       Input_data=DMA_786M1A_vs_HK2,
                       x= "Log2(Distance)",
                       OutputPlotName= "786M1A versus HK2",
...
MetaProViz::VizVolcano(Plot_Settings="Standard",
                       Plot_SettingsInfo= c(individual="CoRe", shape="CoRe_specific", color="Pathway"),
                       Plot_SettingsFile= MetaData_Metab,
                       Input_data=DMA_786M1A_vs_HK2,
                       x= "Log2(Distance)",
                       OutputPlotName= "786M1A versus HK2",
...
#Get the other comparison we are interested in to compare to
DMA_786O_vs_HK2 <- DMA_Annova[["DF"]][["DMA_result"]][["786-O_vs_HK2"]]

#Make the plot
MetaProViz::VizVolcano(Plot_Settings="Compare",
                       Input_data=DMA_786M1A_vs_HK2,
                       AdditionalInput_data=DMA_786O_vs_HK2,
                       x= "Log2(Distance)",
...
```{r, eval=FALSE}
#Get the data
ORA_786M1A_vs_HK2_Consumed <- DM_ORA_res[["786-M1A_vs_HK2"]][["Consumed"]]

#Rename metabolite column (since here we need to use the KEGG metabolite names!)
DMA_786M1A_vs_HK2_KEGG <-DMA_786M1A_vs_HK2[,-1]%>%#remove the original metabolite names
...
MetaProViz::VizVolcano(Plot_Settings="PEA",
                       ...
                       Plot_SettingsFile= KEGG_Pathways,#Must be the pathways used for pathway analysis
                       Input_data= DMA_786M1A_vs_HK2_KEGG,# Must be the data you have used as an input for the pathway analysis
                       AdditionalInput_data= ORA_786M1A_vs_HK2_Consumed,#Must be the results of the pathway analysis
                       x= "Log2(Distance)",
                       OutputPlotName= "KEGG",
                       Subtitle= "PEA" ,
                       SelectLab = NULL)
