...
Pool_Estimation_result<- MetaProViz::PoolEstimation(data = PoolData,
...
Pool_Estimation_result<- MetaProViz::PoolEstimation(data = Intra[,-c(1:3)],
...
```{r, eval=FALSE}
PreprocessingResults <- MetaProViz::PreProcessing(data=Intra[-c(49:58) ,-c(1:3)], #remove pool samples and columns with sample information
...
```{r}
Intra_Preprocessed <- MetaProViz::ReplicateSum(data=Intra_Preprocessed[,-c(1:4)],
...
for(comparison in comparisons){
  ...
  #Perform ORA
  DM_ORA_res[[comparison]] <- MetaProViz::StandardORA(data= DMA%>%remove_rownames()%>%tibble::column_to_rownames("Metabolite"), #Input data requirements: column `t.val` and column `Metabolite`
...
MC_ORA_result <- MetaProViz::ClusterORA(data=MCAres[["MCA_2Cond_Results"]]%>%tibble::column_to_rownames("Metabolite"),
...
MetaProViz::VizPCA(data=Input_PCA,
                   )
```
...
MetaProViz::VizPCA(metadata_info= c(color="Biological_Replicates"),
...
MetaProViz::VizPCA(metadata_info= c(color="Conditions", shape="Biological_Replicates"),
                   metadata_sample= MetaData_Sample,
                   data=Input_PCA,
...
MetaProViz::VizPCA(metadata_info= c(color="Status"),
                   metadata_sample= MetaData_Sample,
                   data=Input_PCA,
...
MetaProViz::VizPCA(metadata_info= c(color="Celltype", shape="Status"),
                   metadata_sample= MetaData_Sample,
                   data=Input_PCA,
                  plot_name = "Cell type")
```
...
\
```{r, eval=FALSE}
MetaProViz::VizSuperplot(data =Input_Superplot[,c(1:6)],#We just plot six metabolites
                                           metadata_sample =MetaData_Sample,
...
MetaProViz::VizSuperplot(data =Input_Plot%>%select(all_of(Metabolite_list)),#We just plot selected metabolites
                                           metadata_sample =MetaData_Sample%>% dplyr::filter(Conditions %in% c("HK2", "786-O", "786-M1A")),
                                           metadata_info = c(Conditions="Conditions", Superplot = "Biological_Replicates"),
                                           plot_type = "Bar", #Bar, Box, Violin
                                           PlotConditions = c("HK2", "786-O", "786-M1A"),#sets the order in which the samples should be plotted
                                           StatComparisons = list(c(1,2),c(1,3), c(2,3)))#Stat comparisons to be included on the plot
```
\
...
MetaProViz::VizVolcano(data=DMA_786M1A_vs_HK2%>%tibble::column_to_rownames("Metabolite"))
```
\
...
MetaProViz::VizVolcano(plot_types="Compare",
                       ...
                       ComparisonName= c(data="786M1A_vs_HK", data2= "786-O_vs_HK2"),
