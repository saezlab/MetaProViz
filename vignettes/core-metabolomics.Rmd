...
```{r, eval=FALSE}
Pool_Estimation_result<- MetaProViz::PoolEstimation(data = Media[,-c(1:3)],
...
PreProcessing_res <-  MetaProViz::PreProcessing(data=Media_input,
...
# Save
Plot_Sized <- MetaProViz:::PlotGrob_Heatmap(InputPlot=heatmap, metadata_info=NULL, metadata_sample=NULL, metadata_feature=NULL,plot_name= "test")
Plot_Sized %<>%
...
for(comparison in comparisons){
  ...
  DM_ORA_res[[comparison]] <- MetaProViz::ClusterORA(data=DMA,
...
MCA_core_res <- MetaProViz::MCA_core(data_Intra =Intra_DMA_786M1A_vs_HK2%>%rownames_to_column("Metabolite") ,
                                     data_core = DMA_786M1A_vs_HK2,
                                     metadata_info_Intra=c(ValueCol="Log2FC",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=0.8),
                                     metadata_info_core=c(DirectionCol="core", ValueCol="Log2(Distance)",StatCol="p.adj", StatCutoff= 0.05, ValueCutoff=28),
...
Plot_Sized <-  MetaProViz:::plotGrob_Superplot(InputPlot=Plot_RG2, metadata_info=c(Conditions="Conditions"), metadata_sample=ClusterSummary_RG2%>%dplyr::rename("Conditions"="SiRCle cluster Name"), plot_name = "SiRCle cluster for RG2_Significant", Subtitle = NULL, plot_type="Bar")
...
Plot_Sized <-  MetaProViz:::plotGrob_Superplot(InputPlot=Plot_RG3, metadata_info=c(Conditions="Conditions"), metadata_sample=ClusterSummary_RG3%>%dplyr::rename("Conditions"="SiRCle cluster Name"), plot_name = "SiRCle cluster for RG3_Change", Subtitle = NULL, plot_type="Bar")
...
MetaProViz::VizHeatmap(data = Input_Plot,
...
MetaLinksDB_Res <- MetaProViz::metsigdb_metalinks(cell_location =c("Extracellular"),
...
MetaLinksDB_Res[["MetalinksDB_Select"]] <- MetaLinksDB_Res[["MetalinksDB"]]%>%
...
```{r, echo=FALSE}
MetaLinksDB_Res[["MetalinksDB_BothDOWN"]] <-merge(x=MetaLinksDB_Res[["MetalinksDB_Select"]],
                                                  y=MCA_res[,c(1,14,48)]%>% dplyr::filter(RG2_Significant=="Both_DOWN (Released/Consumed)") ,
                                                  by.x="hmdb",
                                                  by.y="core_DF_HMDB",
                                                  all.y=TRUE)

# write.csv(MetaLinksDB_Res[["MetalinksDB_BothDOWN"]], "MetalinksDB_BothDOWN.csv")
...
message("The following metabolites are not connected to any receptor or transporter in the MetalinksDB: ",
        MetaLinksDB_Res[["MetalinksDB_BothDOWN"]] %>%
...
```{r}
MC_ORA_result<- MetaProViz::ClusterORA(data=MCA_core_res[["MCA_core_Results"]]%>%column_to_rownames("Metabolite"),
...
MetaProViz::VizSuperplot(data =Input_Superplot%>%  select(any_of(metabolite_list)),#We just plot selected metabolites
                                           metadata_sample =MetaData_Sample,
                                           metadata_info = c(Conditions="Conditions", Superplot = "Biological_Replicates"),
                                           plot_type = "Bar", #Bar, Box, Violin
...
MetaProViz::VizVolcano(data=DMA_786M1A_vs_HK2%>%column_to_rownames("Metabolite"),
                       x= "Log2(Distance)")
```
\
