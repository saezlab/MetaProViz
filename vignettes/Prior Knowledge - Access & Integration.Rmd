...
--> Macabe your vignette: here we can also just make a note to the next chapter Translate IDs
--> We could consider to show two PK examples, once the biocrates kit linked to e.g. MetaLinksDB and classyfire chemical class
--> For MetaLinksDB, we could show all feature metadata examples (cells --> increased space before/after adding multiple IDs, Biocrates)
--> Also show the coverage of the different ID types (some peaks do not have one ID type, but another one! --> True also in other example fetaure metadata, not only biocrates.) --> helper (not exported into RefactorPriorKnowledge.R, and call this plot in TranslateID --> check if the plotting works in 4. )
