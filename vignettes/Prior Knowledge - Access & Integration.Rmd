...
`MetaProViz:::GenerateUpset()` helper function, which will generate an Upset plot showing the overlap of coverage (similar to how a Venn diagram works, but better able to visualize multiple combinations clearly).

-> Macabe: Add all of this restructuring into the function that makes the UpSetPlot, so the user can pass a list of PK resources they want to compare and via SettingsInfo the columns name with gene-metab names. return both, the plot and the summary table created. The GenerateUpset can stay a helper function and what I describe could be MetaProViz::Compare. This can be an exported function and we should add some tests for the input. Additionally as used below, we can add the UpSetPlot automatically into the TranslateID function. For both plots: Adapt using our ggplot helper functions for saving!
-> Macabe / Christina: Add gene-information such as metabolic enzyme, transporter, receptor or other to the gene lists and to the upset plot
```{r}
...
--\> Christina: CountEntries function review.
Is this needed or have we already done this elsewhere (CheckMatchID)?
...
--> Christina: Is it properly exported and documented?
```{r}
...
--> Macabe: Lets add the stacked bar into CheckMatchID and return the plot there automatically.
