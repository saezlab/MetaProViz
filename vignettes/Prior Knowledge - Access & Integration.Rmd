...
- 1. To understand detected metabolite IDs in measured data.\
...
- 5. To perform pathway enrichment analysis.\
...
Example\
--\> Macabe: Load Biocrates data (explain briefly why multiple IDs are present and why this is good practice)
...
--\> Macabe your vignette: here we can also just make a note to the next chapter Translate IDs --\> We could consider to show two PK examples, once the biocrates kit linked to e.g.
MetaLinksDB and classyfire chemical class --\> For MetaLinksDB, we could show all feature metadata examples (cells --\> increased space before/after adding multiple IDs, Biocrates) --\> Also show the coverage of the different ID types (some peaks do not have one ID type, but another one! --\> True also in other example fetaure metadata, not only biocrates.) --\> helper (not exported into RefactorPriorKnowledge.R, and call this plot in TranslateID --\> check if the plotting works in 4. )
...
For this vignette, we will focus on:

-   CellLine experimental data to Hallmarks PK

-   Biocrates experimental data to MetalinksDB PK

## CellLine data to Hallmarks, or Biocrates to Classyfyre?
...
\^ Note that these numbers are minimums, the real values are a little higher once you add for instance metabolites with just CHEBI or just LIMID, etc.

\^ Note also that this isn't considering what the values of these IDs are exactly; maybe some of the HMDB ids are shared for instance for a single metabolite.
...
```{r, echo=FALSE}
# Function to count entries and record NA information
count_entries_with_na <- function(data, column, delimiter = ",") {
  data %>%
    mutate(
      was_na = is.na(.[[column]]) | .[[column]] == "", 
      entry_count = sapply(.[[column]], function(cell) {
        if (is.na(cell) || cell == "") {
...
columns_to_inspect <- c("HMDB")  # Could also include "CHEBI", "LIMID"

for (col in columns_to_inspect) {
  result <- count_entries_with_na(FeatureMetadata_Biocrates, col)
  
  plot <- ggplot(result, aes(x = entry_count, fill = id_label)) +
    geom_histogram(binwidth = 1, boundary = -0.5, color = "black") +
    scale_fill_manual(values = c(
      "No ID" = "#FB8072",       # soft red
      "Single ID" = "#B3DE69",  # soft green
      "Multiple IDs" = "#80B1D3"    # soft blue
    )) +
    labs(title = paste("Number of", col, "IDs per Biocrates Cell"),
         x = "Number of Entries",
         y = "Frequency",
         fill = "Cell Type") +
    theme_minimal() +
...
Now finally we can connect the experimental Biocrates table to the MetalinksDB PK using the HMDB sources.

```{r}
MetaProViz:::CheckMatchID(InputData = FeatureMetadata_Biocrates,
                     PriorKnowledge = MetaLinksDB_Res$MetalinksDB,
                     SettingsInfo = c(InputID="HMDB", PriorID="hmdb", GroupingVariable=NULL))
```

But because we have multiple HMDB IDs per individual Biocrates metabolite in many cases, we also want to take this into account when assessing coverage.
We do this by counting a link to the PK for any of the HMDB IDs for an individual metabolite only once.
