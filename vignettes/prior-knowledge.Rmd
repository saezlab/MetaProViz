...
pk_comp_res <- MetaProViz::compare_pk(data = list(Hallmarks = as.data.frame(Hallmarks_GeneMetab[["GeneMetabSet"]]),
                                                  Gaude = as.data.frame(Gaude_GeneMetab[["GeneMetabSet"]]),
                                                  MetalinksDB = as.data.frame(MetaLinksDB_Res[["MetalinksDB"]]))
                                     )
```
\
...
pk_comp_res_biocft <- MetaProViz::compare_pk(data = list(Biocft = FeatureMetadata_Biocrates), 
...
Biocrates_to_MetalinksDB <- MetaProViz::check_match_id(data = FeatureMetadata_Biocrates,
                                                       input_pk = MetaLinksDB_Res$MetalinksDB,
...
p <- MetaProViz:::viz_stackedbar(
  ...
  plot_name = "Mapping status between Biocrates and MetalinksDB \nusing HMDB, grouped by metabolite class",
  x_label = "Frequency",
  y_label = "Class",
...
p <- MetaProViz:::viz_stackedbar(
  ...
  fill_values = fill_vals,
  fill_labels = fill_lbls,
  plot_name = "Comparison of successful mapping status between Biocrates and MetalinksDB \nusing either single or multi HMDB, grouped by metabolite class",
  x_label = "Frequency",
  y_label = "Class",
  legend_position = c(0.95, 0.05)
)
...
```{r, echo=FALSE, fig.width=8, fig.height=6}
pk_comp_res_keggtrans <- MetaProViz::compare_pk(data = list(kegg_translated = KEGG_Pathways_Translated$TranslatedDF),
