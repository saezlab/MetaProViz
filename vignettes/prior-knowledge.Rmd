...
```{r, echo=FALSE}
Pathways <- alanine_pathways[,2:6]

Count <- Pathways %>%
  tidyr::unite(Pathway, c("pathwaySource", "pathway_name"), sep = ": ")%>%
...
Alanine_Path <- Pathways%>%
  ...
  dplyr::summarise(PathwayCount = dplyr::n_distinct(pathway_name),
...
# Add equivalent IDs:
FeatureMetadata_Cells_AddIDs <- MetaProViz::equivalent_ids(data= Input_HMDB,
...
FeatureMetadata_Cells_AddIDs [2:8, 1:3]%>%
  kableExtra::kbl(caption = "Preview of the DF `FeatureMetadata_Cells_AddIDs` including new columns with potential additional HMDB IDs assign using MetaProViz::equivalent_ids() function.") %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria", font_size = 12)
```
...
```{r}
MetaProViz::metsigdb_chemicalclass()
```
...
MetaProViz::metsigdb_hallmarks()

MetaProViz::metsigdb_gaude()
```
...
Hallmarks_GeneMetab <- MetaProViz::make_gene_metab_set(input_pk=Hallmark_Pathways,
...
```{r}
MetaLinksDB_Res <- MetaProViz::metsigdb_metalinks()
```
...
```{r, echo=FALSE}
# Check how our data looks like:
MetaLinksDB_Res[["MetalinksDB"]][c(12:13,17:18,432,466),c(1:2,6:8, 12:15)]%>%
...
As an example, we will compare the resources containing gene-metabolite sets (Hallmarks, Gaude, MetalinksDB) using `MetaProViz::compare_pk()` function, which will generate a combined datatable and visualises it as an Upset plot. The Upset plot shows the overlap of coverage, similar to how a Venn diagram works, but enable to visualize multiple combinations clearly.
```{r}
# Compare the PK resources
pk_comp_res <- MetaProViz:::compare_pk(data = list(Hallmarks = "Hallmarks",
                                                      Gaude = "Gaude",
...
Between Hallmarks and Gaude we observe a high overlap with metabolites, which can be explained since both gene sets are assigned metabolites by using the same metabolic pathways in the backend of `MetaProViz::make_gene_metab_set` and it is important to keep in mind that this dictates the gene-metabolite associations added.
...
pk_comp_res_biocft <- MetaProViz:::compare_pk(data = pk_list, metadata_info = metadata_info,
...
result_bioc_hmdb_count <- MetaProViz:::count_ids(FeatureMetadata_Biocrates, "HMDB")
...
But for now, we will proceed with making the connections to the PK using all HMDBs, which the MetaProViz function `check_match_id()` has been designed to handle.

## 3. Make connection to PK and assess overlap

Here we connect the experimental Biocrates table to the MetalinksDB PK using the HMDB IDs using the `MetaProViz::check_match_id()` function.\

```{r}
Biocrates_to_MetalinksDB <- MetaProViz:::check_match_id(data = FeatureMetadata_Biocrates,
                     prior_knowledge = MetaLinksDB_Res$MetalinksDB,
...
-   `data_Matched`: a summary table with links to the prior knowledge.
    This is however with any NA values or duplicates of the InputID (e.g. 'HMDB') removed from the table.

-   `data_Matched_NA_and_duplicates`: the same kind of summary table but with all NA and duplicates included.
    Helpful for plotting.

-   `data_Matched_only_NA`: the same kind of summary table but with only rows where the InputID was NA.
    Helpful for inspecting which rows were NA.

<!-- -->

-   `data_Matched_only_duplicates`: the same kind of summary table but with only rows where the InputID was duplicated (or triplicated, etc). Helpful for inspecting which IDs on the input side were duplicated.
...
The `MetaProViz::check_match_id()` has been designed with this in mind, by default splitting any comma separated values in the InputID (or PriorID) into separate entities, counting the number of links between these, and reporting this to the user.

Let's take a look at the data_Matched data:

```{r, echo=FALSE}
irrelevant_cols <- c('INCHI','SMILES','Key','IUPAC','Molecule','MESH','TrivialName_Prior2023', 'Detection','CID')

# Check how our data looks like:
Biocrates_to_MetalinksDB$data_Matched[c(1:8),] %>%
...
```{r, echo=FALSE}
# Check how our data looks like:
table_to_show <- Biocrates_to_MetalinksDB$data_Matched_only_duplicates
table_to_show[c(1:nrow(table_to_show)),] %>%
  dplyr::select(-any_of(irrelevant_cols)) %>%
  kableExtra::kbl(caption = "Preview of the DF `data_Matched_only_duplicates` showing coverage of identifiers (some columns hidden)", row.names=FALSE) %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria", font_size = 12)
```
...
p <- MetaProViz:::viz_stackedbar(
  data = Biocrates_to_MetalinksDB$data_Matched_NA_and_duplicates,
  group_col = "Class",
...
Biocrates_to_MetalinksDB_singleHMDB <- MetaProViz:::check_match_id(data = FeatureMetadata_Biocrates_singleHMDB,
                     prior_knowledge = MetaLinksDB_Res$MetalinksDB,
...
# Call the function with desired parameters
p <- MetaProViz:::viz_stackedbar(
  data = Biocrates_to_MetalinksDB_singleHMDB$data_Matched_NA_and_duplicates,
  group_col = "Class",
  fill_col = "found_match_in_PK",
  fill_values = fill_vals,
  fill_labels = fill_lbls,
...
Biocrates_to_MetalinksDB_foundPKmatch <- Biocrates_to_MetalinksDB$data_Matched_NA_and_duplicates %>%
  filter(found_match_in_PK == TRUE)
Biocrates_to_MetalinksDB_singleHMDB_foundPKmatch <- Biocrates_to_MetalinksDB_singleHMDB$data_Matched_NA_and_duplicates %>%
  filter(found_match_in_PK == TRUE)
...
If we want to convert or 'translate' those IDs to another commonly used form of ID, for instance because our measured data uses another type of ID, we can make use of the `MetaProViz::translate_id()` function.
...
- PubChem\
\
As an example we are translating the KEGG pathways we loaded with `MetaProViz::metsigdb_kegg` into HMDB ids:\

```{r, echo=TRUE, message=FALSE, warning=FALSE}
KEGG_Pathways_Translated <- MetaProViz::translate_id(data= KEGG_Pathways,
...
```{r, echo=FALSE, fig.width=8, fig.height=6}
pk_list <- list(kegg_translated = KEGG_Pathways_Translated$TranslatedDF)
metadata_info <- list(kegg_translated = c("hmdb", "pubchem"))

pk_comp_res_keggtrans <- MetaProViz:::compare_pk(data = pk_list,
                               metadata_info = metadata_info,
                               plot_title = "IDs available after KEGG ID Translation")

pk_comp_res_keggtrans$upset_plot
```
...
Because of this complexity the output of `MetaProViz::translate_id()` includes not only the Translation table showcased above, but additionally information about the mapping ambiguity as well as a summary of the relationships between the Original and Translated IDs.\
...
Within the `MetaProViz::translate_id()` you can set the parameter `summary =TRUE` or in case you have a dataframe that includes both, original and translated ID, you can use the function `MetaProViz::mapping_ambiguity()` to generate this long summary as well as the mapping summary in general.\

```{r, eval=FALSE, include=TRUE}
# Option 1:
KEGG_Pathways_TranslatedSum <- MetaProViz::translate_id(data= KEGG_Pathways,
                                                    metadata_info = c(InputID="MetaboliteID", grouping_variable="term"),
                                                    from = c("kegg"),
                                                    to = c("hmdb", "pubchem"),
                                                    summary =TRUE)
...
```{r}
# Option 2:
MappingProblems <- MetaProViz::mapping_ambiguity(
    data =
...
1.
Over Representation Analysis (ORA) that determines if a set of features (=metabolic pathways) are over-represented in the selection of features (=metabolites) from the data in comparison to all measured features (metabolites) using the Fishers exact test `MetaProViz::cluster_ora`.
...
We offer ORA `MetaProViz::standard_ora`, but there are many other statistical tests that can be used for enrichment analysis.
