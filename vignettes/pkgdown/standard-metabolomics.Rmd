...
```{r pool-estimation, echo=TRUE, warning=FALSE,  fig.width=6, fig.height=4.5, fig.align="left"}
#### Select Pool samples:
# Get the Pool data
...
\
<div class="vscroll-plot">
```{r pool-estimation-2, echo=TRUE, warning=FALSE,  fig.width=6, fig.height=4.5, fig.align="left"}
#--------------------------------------------------------------------
...
<div class="vscroll-plot">
```{r code-5, echo=TRUE,   fig.width=6, fig.height=4.5, fig.align="left"}
...
<div class="vscroll-plot">
```{r dma, warning=FALSE, echo=TRUE, fig.width=7, fig.height=5, fig.align="left"}
...
```{r code-11, echo=FALSE}
# Check how our data looks like:
MappingInfo[1:6,]%>%
kableExtra::kbl(caption ="Pathway Input for MC_ORA.") %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```

```{r cluster-ora, include=FALSE, warning=FALSE}
MC_ORA_result <- cluster_ora(data=MCAres[["MCA_2Cond_Results"]]%>%tibble::column_to_rownames("Metabolite"),
...
```
\
Moreover, we can use MetaData for our features (=Metabolites), which we loaded with the `MappingInfo` and we can also add the information on which cluster a metabolite was assigned to in the `MCA()` analysis above:\
```{r code-13}
...
# Check how our data looks like:
Input_PCA[,1:7]%>%
kableExtra::kbl(caption = "Input_data for `viz_pca()`, with samples as rownames and metabolites as column names.") %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
\
Now lets check out the standard plot:
```{r viz-pca-2, warning=FALSE, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Standard Settings."}
...
```{r viz-pca-3, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do we have a batch effect?"}
...
```{r viz-pca-4, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do the samples cluster for the conditions?"}
...
```{r viz-pca-5, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do the samples cluster for the Cell status?"}
...
```{r viz-pca-6, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do the samples cluster for the Cell type?"}
...
# Check how our data looks like:
Input_Heatmap[,1:7]%>%
kableExtra::kbl(caption = "Input for `viz_heatmap()`, with samples as rownames and metabolites as column names.") %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
\
...
```{r viz-heatmap-2, fig.align="left", fig.cap="Overview heatmap."}
viz_heatmap(data = Input_Heatmap,
plot_name = "Overview")
```
\
Here we can add as many sample metadata information as needed at the same time:\
```{r viz-heatmap-3, fig.align="left", fig.cap="Colour for sample metadata."}
viz_heatmap(data = Input_Heatmap,
metadata_sample = MetaData_Sample,
...
```{r viz-heatmap-4, fig.align="left", fig.cap="Colour for metabolite metadata."}
...
<div class="vscroll-plot">
```{r viz-heatmap-5, echo=TRUE, fig.width=9, fig.height=7.5, warning=FALSE, fig.align="left"}
viz_heatmap(data = Input_Heatmap,
metadata_sample = MetaData_Sample,
...
```
```{r viz-superplot, echo=FALSE}
# Check how our data looks like:
Input_Superplot[,1:7]%>%
kableExtra::kbl(caption = "Input for `viz_superplot()`, with samples as rownames and metabolites as column names.") %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
...
\
<div class="vscroll-plot">
```{r viz-superplot-2, echo=TRUE, warning=FALSE, fig.align="left", fig.width=7, fig.height=5}
...
\
\
```{r viz-superplot-3, eval=FALSE, include=FALSE}
# Input:
...
viz_superplot(data =Input_Plot%>%select(all_of(Metabolite_list)),#We just plot selected metabolites
...
<div class="vscroll-plot">
```{r viz-superplot-4, echo=TRUE, warning=FALSE, fig.align="left", fig.width=7, fig.height=5}
viz_superplot(data =Input_Superplot[,c(1:6)],#We just plot six metabolites
metadata_sample =MetaData_Sample,
metadata_info = c(Conditions="Conditions", Superplot = "Biological_Replicates"),
...
<div class="vscroll-plot">
```{r viz-superplot-5, echo=TRUE, warning=FALSE, fig.align="left", fig.width=7, fig.height=5}
viz_superplot(data =Input_Superplot[,c(1:6)],#We just plot six metabolites
metadata_sample =MetaData_Sample,
metadata_info = c(Conditions="Conditions", Superplot = "Biological_Replicates"),
...
```{r viz-volcano, echo=FALSE}
# Check how our data looks like:
DMA_786M1A_vs_HK2[1:7,]%>%
kableExtra::kbl(caption = "Input_data for `viz_volcano()` are for example differential analysis results from `dma()`.") %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
\
2. Results from Pathway Enrichment Analysis (PEA): Score (e.g. NES score) and stats\
```{r viz-volcano-2, echo=FALSE}
# Check how our data looks like:
DM_ORA_res[["786-M1A_vs_HK2"]][["ClusterGosummary"]][1:4,-c(1,7)]%>%
kableExtra::kbl(caption = "Input_data for `viz_volcano()` are for example pathway enrichment results from `DM_ORA()`.", row.names = FALSE) %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
...
```{r viz-volcano-3, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results."}
...
```{r viz-volcano-4, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results."}
# Run with default parameter --> only need to provide Input_data and the title we like
viz_volcano(data=DMA_786M1A_vs_HK2%>%tibble::column_to_rownames("Metabolite"),
select_label = NULL)
```
...
```{r viz-volcano-5, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results."}
# Run with default parameter --> only need to provide Input_data and the title we like
viz_volcano(data=DMA_786M1A_vs_HK2%>%tibble::column_to_rownames("Metabolite"),
...
```{r viz-volcano-6, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results colour coded/shaped for metabolic clusters from MCA results."}
...
```{r viz-volcano-7, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results colour coded for metabolic pathways and shaped for metabolic clusters from MCA results."}
...
#### **Comparison**
```{r viz-volcano-8, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Comparison."}
...
<div class="vscroll-plot">
```{r viz-volcano-9, echo=TRUE, fig.align="left", fig.width=7, fig.height=5}
viz_volcano(plot_types="Compare",
...
<div class="vscroll-plot">
```{r viz-volcano-10, echo=TRUE, warning=FALSE, fig.align="left", fig.width=7, fig.height=5}
