...
DMA_HK2_vs_786M1A[c(7,9,11:12,14),]%>%
...
# Check how our data looks like:
MC_ORA_HK2_vs_786M1A_Consumed[c(1:5),-1]%>%
...
```
```{r code-13, echo=FALSE}
# Check how our data looks like:
MCA_res[c(17:18, 29, 46),c(1, 5:7,10:15)]%>%
kableExtra::kbl(caption = "mca_core for the comparison of 786-M1A versus HK2 cells in intracellular and core samples.", row.names=FALSE) %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)

# Check how our data looks like:
Clustersummary[-c(1:26),] %>%
kableExtra::kbl(caption = "mca_core summary of number of metabolites per cluster.", row.names=FALSE) %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
\
...
```{r viz-heatmap, eval=FALSE, include=FALSE}
# Input:
...
y=MCA_res[,c(1,14,48)]%>% dplyr::filter(RG2_Significant=="Both_DOWN (Released)") ,
by.x="hmdb",
by.y="core_DF_HMDB",
all.y=TRUE)
...
metabolite-protein network with `viz_metabolite_protein_network()`. The
metabolite name column is recommended, but only the HMDB column is required.
...
MetaLinks_Network <- viz_metabolite_protein_network(
    feature_metadata = MetaLinks_Features,
    metalinks_df = MetaLinksDB_Select,
    metabolite_col = "Metabolite",
    hmdb_col = "core_DF_HMDB",
    save_plot = NULL,
    print_plot = TRUE,
    label_mode = "all"
)

MetaLinks_Network$plot
```

From this visualization, we gain insight into cellular mechanisms for metabolites of interest. In our case, we can see "betaine" and "asparagine" interactions with transporters, receptors and other metabolic enzymes which are curated in MetalinksDB.  
...
metadata_info=c(ClusterColumn="RG2_Significant",
                                                        ...
                                                        save_table= "csv")
```
```{r code-15, echo=FALSE}
# Check how our data looks like:
MC_ORA_result[["DF"]][["Both_DOWN (Consumed)"]][c(1:3,7),]%>%
kableExtra::kbl(caption ="MC_ORA results for the RG2_Significant cluster `Both_DOWN (Consumed)`.", row.names=FALSE) %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```
\
...
```{r viz-pca-4, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do we have a batch effect?"}
viz_pca(metadata_info= c(color="Biological_Replicates"),
metadata_sample = MetaData_Sample ,
data=Input_PCA,
...
```{r viz-pca-5, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do the samples cluster for the conditions?"}
...
```{r viz-pca-6, fig.align="left", fig.width=6, fig.height=4.5, fig.cap="Figure: Do the samples cluster for the Cell status?"}
...
```{r viz-heatmap-3, fig.align="left", fig.width=10, fig.height=12, fig.cap="Overview heatmap."}
viz_heatmap(data = Input_Heatmap,
plot_name = "Overview")
```
\
Here we can add as many sample metadata information as needed at the same time:\
```{r viz-heatmap-4, fig.align="left", fig.width=10, fig.height=12, fig.cap="Colour for sample metadata."}
viz_heatmap(data = Input_Heatmap,
metadata_sample = MetaData_Sample,
...
```{r viz-heatmap-5, fig.align="left",fig.width=12, fig.height=14, fig.cap="Colour for metabolite metadata."}
viz_heatmap(data = Input_Heatmap,
metadata_sample = MetaData_Sample,
...
<div class="vscroll-plot">
```{r viz-heatmap-6, echo=TRUE, fig.width=9, fig.height=6, warning=FALSE, fig.align="left"}
viz_heatmap(data = Input_Heatmap,
metadata_sample = MetaData_Sample,
...
<div class="vscroll-plot">
```{r viz-superplot-3, echo=TRUE, warning=FALSE, fig.align="left", fig.width=7, fig.height=5}
...
# Check how our data looks like:
DMA_HK2_vs_786M1A[1:7,1:6]%>%
kableExtra::kbl(caption = "Input_data for `viz_volcano()` are for example differential analysis results from `dma()`.") %>%
kableExtra::kable_classic(full_width = FALSE, html_font = "Cambria", font_size = 12)
```

#### **Standard**
Here we will first look into the results from the differential analysis (see section `dma` above) for the comparison of `HK2_vs_786M1A`:
```{r viz-volcano-2, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results."}
...
```{r viz-volcano-6, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying dma results."}
...
```{r viz-volcano-9, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Comparison."}
# Make the plot
viz_volcano(plot_types="Compare",
...
\
<div class="vscroll-plot">
```{r viz-volcano-11, echo=TRUE, warning=FALSE, fig.align="left", fig.width=7, fig.height=5}
