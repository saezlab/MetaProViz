...
\- 4.
[to investigate prior knowledge resources in more depth.](#sect4)

\- 5.
[to translate IDs and assess ambiguities to prior knowledge.](#sect5)

\- 6.
[to take advantage of the translation results in deeper prior knowledge investigation](#sect6)
...
```
\
```{r code-9, eval=FALSE, echo=FALSE, fig.width=8, fig.height=6}
...
## 4. Deeper investigation of MetalinksDB metabolite-protein interactions
Now that we have investigated the coverage of metabolite ID annotations and their connections and matches to Prior Knowledge, we can go a step further and look into the interaction mechanisms of metabolite features with MetalinksDB transporter- and receptor proteins, as well as metabolic enzymes.
For that, we will subset MetalinksDB by the "interaction_family" column, which is subset in one of the following categories: "Other protein-metabolite", "Transporter-metabolite", "Receptor-metabolite", and "Enzyme-metabolite", using the 'viz_metabolite_protein_network()' function.

Due to the amount of all biocrates features and the amount of Prior Knowledge in MetalinksDB, these visualizations may become quite convoluted. Hence, to resemble a possible subset, we will investigate only the amino acid subset of all biocrates features with the listed MetalinksDB subsets.

```{r}
# get metalinksdb subsets bsaed on interaction_family
...
# subset biocrated_features to amino acids
...
Plot the Metabolite Network interaction network of biocrates amino acids and MetalinksDB Receptors
```{r}
# receptor-network
...
Plot the Metabolite Network interaction network of biocrates amino acids and MetalinksDB Transporters
```{r}
# transporter-network
...
Plot the Metabolite Network interaction network of biocrates amino acids and MetalinksDB Metabolic Enzymes
```{r}
...
Note that we enabled the varialble `plot_metabolite_interaction_overlap`, which additionally plots a graph of interaction overlap on the metabolite level.
While the metabolite-protein network plots show specific interactions of metabolites with proteins, we can now also investigate how metabolites share interactions of the same proteins.

For example, in the receptor subset of MetalinksDB, we observe many interactions for Glutamic acid (Glu) and Glycine (Gly). Indeed, the corresponding overlap plot shows that Gly and Glu share 6 proteins which they interact with. Additionally, we see that Glu and Asp (Aspartic acid) share 13 receptors, while Asp has 17 in total and Glu has 110 in total, hinting at a possibly higher receptor specificity of Aspartic acid in comparison to a more versatile interaction of Glutamic acid.
Note however, that metabolite-protein interactions have various interaction properties, binding mechanisms and the Prior Knowledge resources are always biased by over-representation of well-studies small molecules or proteins. Therefore, the overlap graph hints at shared protein interactions, but does not necessarily show that two metabolites have n exact same interactions.
...
# 4. Prior Knowledge resource overlap {#sect4}
:::: {.progress .progress-striped .active}
::: {.progress-bar .progress-bar-success style="width: 100%"}
:::
::::
...
We just saw an example of how we can connect experimental data to prior knowledge. We observed shared patterns between metabolites, having overlapping protein interactions.
We have the possibility to go into even more depth and investigate the metabolite-overlap of prior knowledge resources.
<strong>See the [metsigdb-resouces-composition vignette](https://saezlab.github.io/MetaProViz/articles/pkgdown/metsigdb-resouces-composition) for an in-depth investigation of all MetSigDB resources </strong>
:::

<br>


Using the `cluster_pk()` function, we can plot the resource-level overlap e.g. KEGG pathways, showing how many metabolites are shared between pathways.

```{r}
# load kegg
kegg <- metsigdb_kegg()

# add a counter to kegg, counting the number of metabolites per pathway term
kegg <- kegg |> group_by(term) |> mutate(n_metabolites_in_pathway = n()) |> ungroup()
...
kegg_clustering <- 
    cluster_pk(
        ...
        similarity = "jaccard",
        input_format = "long", 
        threshold = 0.4, # only pathways with higher similarity than 0.4 are considered for clustering
        plot_threshold = 0.5, # only pathway-pathway edges with at lease 0.5 similarity are plotted. Set to 0 to plot all edges (even 1 metabolite overlap)
        clust = "community", # Louvain community detection to determine which pathways are one cluster
        node_size_column = "n_metabolites_in_pathway", # node size will depend on the pathway size
        show_density = TRUE, # colored background behind clusters, for easier identification
        print_plot = TRUE, 
        plot_name = "KEGG_resource_overlap"
    )

```

Now we can see a variety of clusters within KEGG, hinting at a certain level of redundancy between certain pathways. Depending on the parameters we choose for clustering, 
clusters form very strictly (high amount of overlap between pathways needed to be considered the same cluster), or rather losse (showing clustering, even with less-redundant pathway-pathway connections).
At this point in the analysis, we came across a blocking factor: if we want to investigate KEGG pathways in more depth, we have to rely on KEGG IDs. However, the biocrates features have no KEGG IDs, hence we cannot match them to KEGG pathways. Previously, we were able to connect the biocrates features to MetalinksDB, because both used HDMB identifiers. Hence, let's look into ID translation next.
...
![Fig. 2: Mapping problems in prior knowledge metabolite-sets when translating metabolite IDs.](../Fig.2.png){width="80%"}
...
![Fig. 3: Mapping problems in prior knowledge metabolite-sets when translating metabolite IDs and the connection to detected (measured input metabolites).](../Fig.3.png){width="80%"}
...
# 6. Use the translated KEGG prior knowledge together with biocrates features {#sect6}
...
Now that we have gained HMDB IDs to KEGG pathways, we may investigate the KEGG pathway overlap again, this time using a trick:
To set the next step into context: a strong point of the graph plot (and a caveat at the same time) is the impact of different forms of visualizations. While the clustering parameters, thresholds and plotting parameters already can change the graph a lot, we can mutate the node sizes.
We are able to connect experimental data to the KEGG network graph, by representing pathway node sizes larger or smaller, depending on the share of metabolites of a pathway, which is found in our experimental feature space.

Now we may connect the biocrates features to the plot.
```{r}
kegg_translated <- KEGG_Pathways_Translated[["TranslatedDF"]]

# add a counter to kegg, counting the number of metabolites per pathway term
kegg_translated <- kegg_translated |> group_by(term) |> mutate(n_metabolites_in_pathway = n()) |> ungroup()

# Note: biocrates has a previously created single_HDMB version, which we'll use.
#       However, the translated KEGG pathways sometimes have multiple HDMB IDs, so we want to match that accordingly

# extract biocrates single HMDB IDs to match to translated KEGG pathways
biocrates_hmdb <- FeatureMetadata_Biocrates_singleHMDB %>%
    dplyr::pull(HMDB_single) %>%
    stringr::str_trim() %>%
    unique()

# extract the metabolite matched to kegg
kegg_metabolite_matches <- kegg_translated %>%
    dplyr::mutate(kegg_row_id = dplyr::row_number()) %>%
    dplyr::select(
        kegg_row_id,
        term,
        n_metabolites_in_pathway,
        hmdb
    ) %>%
    ...
    dplyr::summarise(
        detected_in_biocrates = any(hmdb %in% biocrates_hmdb),
        .groups = "drop"
    )

# compute the pathway coverage. i.e. How much percent of metabolites from biocrates can we find in each KEGG pathway?
pathway_coverage <- kegg_metabolite_matches %>%
      dplyr::group_by(term) %>%
      dplyr::summarise(
          pct_metabolites_detected =
              100 * sum(detected_in_biocrates) /
              dplyr::first(n_metabolites_in_pathway),
          .groups = "drop"
      )

kegg_translated_matched_with_biocrates <- kegg_translated %>%
    dplyr::left_join(pathway_coverage, by = "term")


# plot the previous version of KEGG clustering, for comparison
print(kegg_clustering$graph_plot)

# run clustering, this time with the node sizes based on biocrates matches
kegg_clustering_biocrates_matched <- cluster_pk(
        kegg_translated_matched_with_biocrates, 
        c(
            metabolite_column = "MetaboliteID",
            pathway_column = "term"
        ), 
        ...
        plot_threshold = 0.5, 
        clust = "community",
        node_size_column = "pct_metabolites_detected", # node size will depend on the % of matched biocrates features
        show_density = TRUE, 
        print_plot = TRUE, 
        plot_name = "KEGG_resource_overlap_boicrates matched"
    )
```

While the clustering parameters stayed the same, we can see that the node sized changed, sometimes drastically. Thereby, we alter the interpretation of clustering results actively and we need to be aware of the potential bias we create. Yet, this nicely allows us to investigate pathways further, in case we can see large nodes: these pathways are well covered by the biocrates features. Therefore, alterations in such well-represented pathways may be more reliable, because a larger number of metabolites must be changed in downstream differential analyses, while smaller pathway nodes (less well-represented by experimental features) may change due to some outlier metabolites. 
In general, the graph visualization is an important tool to get a better understanding of resource composition and possible interpretation bias / changes in downstream analyses. We made another dedicated vignette [metsigdb-resource-composition](https://saezlab.github.io/MetaProViz/articles/metsigdb-resource-composition.html), where we specifically showcase all MetSigDB Prior Knowledge resources in depth and perform more of these graph visualizations, together with many more quantification analyses.


# 7. Following: Run enrichment analysis {#sect7}
...
\
\
[As part of the **MetaProViz** package you can load the example data using `data()`]{style="text-decoration:underline"} to test the enrichment analysis:\
