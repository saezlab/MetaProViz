...
DMA_Annova <- MetaProViz::DMA(Input_data=Intra_Preprocessed[,-c(1:3)], #we need to remove columns that do not include metabolite measurements
...
for(comparison in comparisons){
  ...
  #Perform ORA
  DM_ORA_res[[comparison]] <- MetaProViz::DM_ORA(Input_data=DMA,#Input data requirements: column `t.val` and column `Metabolite`
                                                 PathwayFile=KEGG_Pathways[,-3],#Pathway file requirements: column `term`, `Metabolite` and `Description`. Above we loaded the Kegg_Pathways using MetaProViz::Load_KEGG()
                                                 PathwayName="KEGG",
                                                 minGSSize=3,
                                                 maxGSSize=1000,
                                                 pCutoff=0.01,
...
```{r}
MCAres <- MetaProViz::MCA_2Cond(Cond1_File=DMA_Annova[["DF"]][["DMA_result"]][["786-O_vs_HK2"]],
                      Cond2_File=DMA_Annova[["DF"]][["DMA_result"]][["786-M1A_vs_HK2"]],
...
# Check how our data looks like:
ClusterSummary <- MCAres[["DF"]][["ClusterSummary"]]
```
```{r, echo=FALSE}
# Check how our data looks like:
...
```{r}
#We need a pathway file with the columns Metabolite, term and description
...
# Check how our data looks like:
PathwayInput[1:6,]%>%
  kableExtra::kbl(caption ="Pathway Input for MetaProViz::MC_ORA.", row.names=FALSE) %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria", font_size = 12) 
```
...
```{r}
MetaData_Metab <-merge(MappingInfo[,c(1,4)], MCAres[["DF"]][["MCA_result"]][,c(1,14)], by="Metabolite", all.y=TRUE)
```
```{r, echo=FALSE}
# Check how our data looks like:
...
### Superplots
Intensity plots for each feature (=metabolite) can be useful to visually examine differences among groups.\
...
We can select the conditions that are plotted using the `Selected_Conditions` parameter. Additionally, after selecting some conditions, we can add statistics for comparisons specified in the `Selected conditions` parameter. If only one pair is added in the Selected_Comparisons students t-test  is performed which anova is used for more than one comparison. Finally, plots are exposted in one pdf file. However, individual plots can be exported by setting the parameter `Individual_plot` = TRUE
```{r}
Vizsuperplot(Input_data = Input_Superplot,
             Input_SettingsFile = MetaData_Sample,
             Input_SettingsInfo = c(conditions="Conditions", superplot = "Biological_Replicates"),
...
MetaProViz::VizVolcano(Input_data=DMA_786M1A_vs_HK2)
```
\
...
MetaProViz::VizVolcano(Plot_Settings="Compare",
                       Input_data=DMA_786M1A_vs_HK2,
                       AdditionalInput_data=DMA_Annova[["DF"]][["DMA_result"]][["786-O_vs_HK2"]],
...
MetaProViz::VizVolcano(Plot_Settings="PEA",
                       ...
                       Input_data= DMA_786M1A_vs_HK2,# Must be the data you have used as an input for the pathway analysis
                       AdditionalInput_data=DM_ORA_res[["786-M1A_vs_HK2"]][["ClusterGoSummary"]],#Must be the results of the pathway analysis
                       OutputPlotName= "KEGG",
                       Subtitle= "PEA" ,
                       SelectLab = NULL)
