...
Pool_Estimation(Input_data = PoolData,
...
```
\
The results from the `Pool_Estimation()` is a table that has the Coefficient of variation (CV), Standard Error of the Mean (SEMean) and the Standard Error of the Median (SEMedian). If there is a high variability for those parameters (=TRUE), one should consider to remove those features from the data. For the example data nothing needs to be removed.\
\
...
- Here we should update to KEGG pathways!
...
```{r, echo=FALSE, warning=FALSE, fig.align="left", fig.width=8, fig.height=5}
MetaProViz::MC_ORA(Input_data=MCAres,
                   MetabCluster_lab="RG2_Significant",
                   RemoveBackground=TRUE,
                   PathwayFile=PathwayInput,
...
One of the big advantages of `MetaProViz` visualisation module is that the user can choose the format (e.g. pdf, svg,...) in which the figures should be saved and that they are saved in a publication ready style. This means that the x- and y-axis size will always be adjusted for the amount of sample or features, or in the case of Volcano plot and PCA plot have a fixed sizes. In this way, there is no need for many adjustments and the figures can just be dropped into the presentation or paper.\
\
In general, each of the different plot types (e.g. Volcano plot, Lollipop graph,...) have up to 3 main `Plot_Settings`.\
...
```{r, fig.align="left", fig.cap="Overviw heatmap."}
...
### Volcano plot
Here we will look at all the different options we have to display our results from the different analysis (DMA, MCA, PEA), which will help us to interpret our results as this can be sometimes difficult from the many data tables.

Note: show the input DF --> Log2Fc and p.adj!

#### **Standard**
Here we will first look into the results from the differential analysis (see above 2.2 DMA) for the comparison of `786-M1A_vs_HK2`:
...
We should enable a pre-selection (Which pathways should be plotted) either with a helper function or within the VizVolcano function. 

We should rename AdditionalInput_data?!

If the title of the figure is too long (often when opathway name is long) it is cut-off. This should also be fixed if possible.
\
\
\
