...
```{r}
#Metadata:
MetaData <- Intra_Preprocessed[,c(1:2)]%>%
...
```{r}
Heatmap_data <-  Intra_Preprocessed[,-c(1:3)]#remove columns that include Metadata such as cell type,...
```
```{r, echo=FALSE}
# Check how our data looks like:
Heatmap_data[,1:7]%>%
  kableExtra::kbl(caption = "Inoput for MetaProViz::VizHeatmap(), with samples as rownames and metabolites as column names.") %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria", font_size = 12)
```
\
...
Sometimes it can be advantageous to split the samples onto individual plots:
To do:
