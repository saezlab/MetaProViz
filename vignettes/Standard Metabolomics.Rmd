...
Pool_Estimation_result<- MetaProViz::Pool_Estimation(Input_data = PoolData,
...
Pool_Estimation_result<- MetaProViz::Pool_Estimation(Input_data = Intra[,-c(2:4)] %>% column_to_rownames("Code"),
                            Input_SettingsFile = Intra[,1:4] %>% column_to_rownames("Code"),
                            Input_SettingsInfo = c( PoolSamples = "Pool"),
                            Unstable_feature_remove = FALSE, 
                            Threshold_cv = 100)
```
```{r, echo=FALSE}
Pool_Estimation_result_DF <- Pool_Estimation_result[["DFs"]]
Pool_Estimation_result_DF_CV <-Pool_Estimation_result_DF[["CV_result"]]
...
```{r}
PreprocessingResults <- MetaProViz::Preprocessing(Input_data=Intra_M, 
                                                  Input_SettingsFile=SampleInfo,
                                                  Input_SettingsInfo = c(Conditions = "Conditions", Biological_Replicates = "Biological_Replicates"),
...
Intra_Preprocessed <- PreprocessingResults[["DFs"]][["Preprocessing_output"]][["Processed_data"]]
```
```{r, echo=FALSE}
# Check how our data looks like:
...
#1.PCA plot
P1 <- PreprocessingResults[["Plots"]][["Outlier_plots"]][["PCA_round1"]]
#P1

#2.Screeplot
P2 <- PreprocessingResults[["Plots"]][["Outlier_plots"]][["ScreePlot_round1"]]
#P2

#3.Hotellins Outlier
P3 <-  PreprocessingResults[["Plots"]][["Outlier_plots"]][["HotellingsPlot_round1"]]
P3
...
P4<- PreprocessingResults[["Plots"]][["Outlier_plots"]][["PCA_round2"]]
P4
P5<- PreprocessingResults[["Plots"]][["Outlier_plots"]][["ScreePlot_round2"]]
P5
P6<- PreprocessingResults[["Plots"]][["Outlier_plots"]][["HotellingsPlot_round2"]]
P6
```
\
...
# PCA plot
# 1. Marked Outliers:
Q1 <- PreprocessingResults[["Plots"]][["QC_plots"]][["QC_PCA_and_Outliers"]]
Q1

#2. Marked Biological replicates:
Q2 <- PreprocessingResults[["Plots"]][["QC_plots"]][["QC_PCA_Replicates"]]
Q2
```
\
...
`DMA_786-O_vs_HK2`<- DMA_786O_vs_HK2[["DFs"]][["DMA_result"]]#Get the DMA table
...
`DMA_786-M1A_vs_HK2` <- DMA_786M1A_vs_HK2[["DFs"]][["DMA_result"]]#Get the DMA table
...
`DMA_786-M2A_vs_HK2` <- DMA_786M2A_vs_HK2[["DFs"]][["DMA_result"]]#Get the DMA table
...
MC_ORA_result<- MetaProViz::MC_ORA(Input_data=MCAres[["DFs"]][["MCA_result"]],
...
```{r}
Input_PCA <- Intra_Preprocessed[,-c(1:4)]#remove columns that include Metadata such as cell type,...
```
```{r, echo=FALSE}
# Check how our data looks like:
Input_PCA[,1:7]%>%
...
<div class="vscroll-plot">
```{r,eval=FALSE, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Lolipop displaying metabolite clusters individually"}
MetaProViz::VizLolipop(Input_data =  `DMA_786-M1A_vs_HK2`, 
                       Plot_SettingsInfo= c(color = "t.val", size = "p.val", individual= "RG2_Significant"),
                       Plot_SettingsFile=  MetaData_Metab,
                       OutputPlotName= "786M1A versus HK2",
