...
\
Before we can run the ORA on the DMA results, we 
...
KEGG <- KEGG_Pathways[,c(1,3)]%>%
  dplyr::rename("term"=1,
                "Metabolite"=2)
KEGG$Description <- KEGG$term  

#Input data requirements: column `t.val` and column `Metabolite` (needs to match Pathway file)
#We could use the KEGG-IDs to match, yet we prefer the trivial names
HK2_vs_786M1A <- merge(`DMA_HK2_vs_786-M1A`[,1:5], Pathways[1:3], by="Metabolite", all=TRUE)
HK2_vs_786M1A <-HK2_vs_786M1A[complete.cases(HK2_vs_786M1A),-1]%>%
  dplyr::rename("Metabolite"=6)

HK2_vs_786M2A <- merge(`DMA_HK2_vs_786-M2A`[,1:5], Pathways[1:3], by="Metabolite", all=TRUE)
HK2_vs_786M2A <-HK2_vs_786M2A[complete.cases(HK2_vs_786M2A),-1]%>%
  dplyr::rename("Metabolite"=6)

HK2_vs_786O <- merge(`DMA_HK2_vs_786-O`[,1:5], Pathways[1:3], by="Metabolite", all=TRUE)
...
### Lollipop graphs
Next we will look at another option we have to display our results from the different analysis (DMA, MCA, GSE) and that would be with Lolipop graphs.

#### **Standard**
Here we will first look into the results from the differential analysis (see above 2.2 DMA) for the comparison of `HK2 versus 786M1A` using a plain graph:
```{r, fig.align="left", fig.width=7, fig.height=5, fig.cap="Figure: Standard figure displaying DMA results."}
# Run with default parameter --> only need to provide Input_data and the title we like
...
MetaProViz::VizLolipop(Input_data = DMA_HK2v786O, 
                       Plot_SettingsInfo= c(color="t.val", size="p.adj"), 
                       Plot_SettingsFile= DMA_HK2v786O,
                       OutputPlotName= "HK2 versus 786M1A",
                       Subtitle= "Results of DMA")
```
\
...
SettingsO <- merge(Settings,DMA_HK2v786O, by="Metabolite" )
# Check our settings:
knitr::kable(Settings[1:4,] , caption = "Settings for plots.", row.names=FALSE)
```
...
```{r}
## Compare
MetaProViz::VizLolipop(Input_data = list(DMA_HK2v786O,DMA_HK2v786M1A), 
...
MetaProViz::VizLolipop(Input_data = list(DMA_HK2v786O,DMA_HK2v786M1A),
           ...
           Plot_Settings="Compare", 
           Comparison_name= list("Cond1", "Cond2"), 
           Plot_SettingsFile = list(DMA_HK2v786O,DMA_HK2v786M1A),
           OutputPlotName= "DMA HK2v786O versus DMA HK2v786M1A",
           Subtitle= "Results of DMA")
...
```{r}
SettingsM1A <- merge(Settings,DMA_HK2v786M1A, by="Metabolite" )
knitr::kable(SettingsO[1:4,] , caption = "Settings for plots.", row.names=FALSE)
```

```{r}
VizLolipop(Input_data = list(DMA_HK2v786O,DMA_HK2v786M1A), 
