...
- automatically taken care of in limma --> we do log transformation, so we need untransformed data.

`Plot_SettingsFile_Sample` and `Plot_SettingsFile_Metab`

Noteworthy, if you have not performed missing value imputation and hence your data includes NAs or 0 values for some features, this will be taken into account in the `DMA()` function:\
...
DMA_Annova <- MetaProViz::DMA(Input_data=Intra_Preprocessed[,-c(1:4)], #we need to remove columns that do not include metabolite measurements
                              Input_SettingsFile_Sample=Intra_Preprocessed[,c(1:2)],#only maintain the information about condition and replicates
                              Input_SettingsInfo = c(conditions="Conditions", numerator=NULL, denominator = "HK2"),# we compar all_vs_HK2
                              STAT_pval ="aov",
                              STAT_padj="fdr",
                              Input_SettingsFile_Metab = MappingInfo,# Adds metadata for the metabolites such as KEGG_ID, Pathway, retention time,...
                              OutputName='Annova')

#rename to Input_SettingsFile_Sample and Input_SettingsFile_Metab

#DMA_Annova[["DF"]][["DMA_result"]][["786-M1A_vs_HK2"]]
...
Shapiro <- DMA_786O_vs_HK2[["Shapiro_Results"]]
...
# Check how our data looks like:
Shapiro[,1:8]%>%
  kableExtra::kbl(caption = "Preview of the Shaprio results for the comparison of HK2 versus 786-O cells.", row.names=FALSE) %>%
  kableExtra::kable_classic(full_width = F, html_font = "Cambria", font_size = 12) #%>%
  #kableExtra::scroll_box(width = "100%", height = "200px")
```
\
...
MCA_2Cond <- MetaProViz::MCA_rules(data="2Cond")
```
