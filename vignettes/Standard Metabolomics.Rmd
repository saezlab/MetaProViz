...
Include some description about saving the plots in publication ready format!
\
...
MetaProViz::VizPCA(Plot_SettingsInfo= c(color="Celltype", shape="Status"),
                   ...
                   Save_as_Results = "csv")#file format
```
...
```
\
Now we can generate an overview heatmap. Since we plot all metabolites the metabolite names are not plotted since this would get too crowded (You can enforce this by changing the parameter `enforce_FeatureNames` to `TRUE`).
```{r, fig.align="left", fig.width=8, fig.height=9.6}
MetaProViz::VizHeatmap(Input_data = Input_Heatmap, 
                       OutputPlotName = "Overview")
```
\
Here we can add as many Sampleinformation from our MetaData as we want at the same time:\
```{r, fig.align="left", fig.width=10.3, fig.height=10.6}
# col annotation: Colour for samples
MetaProViz::VizHeatmap(Input_data = Input_Heatmap,
...
```{r, eval=FALSE}
# individual: One individual plot for each pathway
MetaProViz::VizHeatmap(Input_data = Input_Heatmap,
                       Plot_SettingsFile_Sample= MetaData_Sample,
...
MetaProViz::VizHeatmap(Input_data = Input_Heatmap, 
                       Plot_SettingsFile_Sample = MetaData_Sample,
                       Plot_SettingsInfo = c(individual = "RG2_Significant", color_Sample = list("Conditions","Biological_Replicates"), color_Metab = list("Pathway")),
                       Plot_SettingsFile_Metab =  MetaData_Metab,
                       OutputPlotName = "MCA-Cluster")

```

To do:
* Sometimes it can be advantageous to split the samples onto individual plots: Implement that we can also use individual for samples
* If we have a long legend as for pathways its cut-off in the saving process: Is there a way to ensure the legend is set left from the other legends?
...
MetaData_Metab <- merge(MetaData_Metab,`DMA_HK2_vs_786-O`[,-6], by="Metabolite" )
```
